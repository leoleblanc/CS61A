...
(yello)
; expect Error: unknown identifier: yell0
...
(cons 'banana 'muffins)
; expect (banana . muffins)

(and 'the_power_of_science)
;expect the_power_of_science
...
(begin (display 23) (* 9 222) (print 28))
; expect 2328 ; okay

(define g (lambda (y z) (* y z)))
; expect g
...
(if (> 4 8) 'yes 'no 'maybe)
; expect Error: too many operands in form (if ought to have a true and false result)

(if (and #f (/ 1 0)) (/ 100 0) 'avoided_disaster)
; expect avoided_disaster
...
(cond ((> 2 4) 'wut) ((= 3 6) 'naw) (else 'nonsense))
; expect nonsense

(begin 2 3 4)
; expect 4

(lambda (x) '(Y2K x) (+ x 2000))
; expect (lambda (x) (begin (quote (y2k x)) (+ x 2000)))

(lambda () 17)
; expect (lambda () 17)

(lambda () 2 17)
; expect (lambda () (begin 2 17))

(define (super_saiyan g) (+ g 9000))
; expect super_saiyan

super_saiyan
; expect (lambda (g) (+ g 9000))
...
(begin (define f (lambda (x y) (+ (* x y) (- x y)))) 'oops)
; expect oops

(define f (mu (x) (* x y z)))
; expect f

(define h (lambda (x y z) (f (+ z x))))
; expect h
