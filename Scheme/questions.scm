...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (construct-partitions total current-pieces current-value lst)
    (cond ((= total 0) (list lst))
	  ((or (= (length lst) max-pieces) (< total 0) (< current-value 1)) nil)
	  (else (append (construct-partitions (- total current-value) current-pieces current-value (cons current-value lst))
			(construct-partitions total current-pieces (- current-value 1) lst)))))
    (construct-partitions total max-pieces max-value nil))
...
(define (tree-sums tree)
  ...
  (if (null? (children tree))
	     (list (entry tree))
  (map (lambda (node) (+ (entry tree) node))
