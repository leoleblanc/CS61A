...
#        raise SchemeError("unknown identifier: {0}".format(str(symbol)))
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("too many/few arguments given")
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        lst_args = list(args) #converts to python list
        if self.use_env == True: #appends environment to argument list
            lst_args.append(env)
        try:
            val = self.fn(*lst_args)
        except TypeError:
            raise SchemeError("bad format for arguments")
        return (val, None)
...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            "*** YOUR CODE HERE ***"
            new_frame = self.env.make_call_frame(self.formals, args) #creates a new frame with params bound to arguments
            answer = scheme_eval(self.body, new_frame) #evaluates the expression (body of procedure) in the new frame
            return answer, None #returns the value, and None, as always
...
class MuProcedure(LambdaProcedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            answer = scheme_eval(self.body, new_frame)
            return answer, None
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    "*** YOUR CODE HERE ***"
    if len(vals) > 2:
        return function_type(formals, Pair("begin", vals.second), env), None
    return function_type(formals, vals[1], env), None
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        expression = vals.second
        values = Pair(arguments, expression)
        env.define(target.first, do_lambda_form(values, env)[0])
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for expression in bindings: #add the necessary bindings
        if len(expression) > 2:
            raise SchemeError
        names = Pair(expression[0], names)
        values = Pair(scheme_eval(expression[1], env), values)
...
def do_and_form(vals, env):
    ...
    if len(vals) == 0:
        ...
        for each in range(len(vals)):
            result = scheme_eval(vals[each], env)
            if not result:
                return scheme_false, None
            elif each == len(vals) - 1:
                return result, None
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            return scheme_eval(clause.second[0], env), None
    return okay, None
...
def check_formals(formals):
    ...
    for parameter in formals:
        ...
        elif parameter in checked:
            raise SchemeError("Symbol repeated")
        else:
            checked.append(parameter)
