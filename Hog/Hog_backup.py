...
def make_averaged(fn, num_samples=1000):
    ...
    def average(*args):
        ...
        new_result = 0
        total = 0
        while num_samples - n > 0:
            result = fn(*args)
            new_result += result
            n += 1
        return new_result / num_samples
    return average
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    temp_max = 0
    num_dice = 0
    max_score, min_dice = 0, 0
    while rolls < 10:
        if temp_max > max_score:
            max_score = temp_max
            min_dice = rolls
            rolls += 1
            temp_max = (make_averaged(roll_dice)(rolls, dice))
            num_dice = temp_max / dice()
            print(rolls, "dice scores", temp_max, "on average")
        else:
            rolls += 1
            temp_max = (make_averaged(roll_dice)(rolls, dice))
            num_dice = temp_max / dice()
            print(rolls, "dice scores", temp_max, "on average")
    if rolls == 10 and temp_max > max_score:
        max_score = temp_max
        min_dice = rolls
        rolls += 1
    print(min_dice)
