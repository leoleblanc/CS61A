...
# Name: Derrick Hu, Leo LeBlanc
# Email: HuDerrick@berkeley.edu, lleblanc@berkeley.edu
...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant == None:
                ...
                self.ant = insect
            elif insect.can_contain(self.ant) and self.ant.container == False:
                insect.contain_ant(self.ant)
                self.ant = insect
            elif insect.can_contain(self.ant) == False and self.ant.can_contain(insect):
    ...
    def remove_insect(self, insect):
        ...
        if insect.removable != False:
            if not insect.is_ant():
                self.bees.remove(insect)
            else:
                assert self.ant == insect, '{0} is not in {1}'.format(insect, self)
                if hasattr(self.ant, 'ant'):
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        "*** YOUR CODE HERE ***"
        if self.place.ant is None or self.place.ant.blocks_path != True:
            return False
        else:
            return True
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container == True and self.ant == None and other.container != True:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for number in range(self.min_range):
            present_place = present_place.entrance
        while present_place != None and present_place != hive and distance <= self.max_range:
            if len(present_place.bees) > 0:
                return random_or_none(present_place.bees)
            present_place = present_place.entrance
            distance += 1
        return None
...
class Hive(Place):
    ...
    def strategy(self, colony):
        ...
        # exits is a list. We add to this list the elements in colony.places.values() \
        # colony.places is a dictionary/ ordered dictionary. \
        # Keys are the place names, values are the actual places. \
        # However the if statement at end means we \
        # only add to list if place's entrance is itself, MEANING it has an exit by Q3
        for bee in self.assault_plan.get(colony.time, []):
            bee.move_to(random.choice(exits))
            # so now the hive's exit is a random place's exit, which by our list \
            # only has places with exits to go down.
...
class AntColony:
    ...
    def configure(self, hive, create_places):
        ...
        def register_place(place, is_bee_entrance):
            self.places[place.name] = place    #place.name is the key, actual place is the value
            if is_bee_entrance:
                place.entrance = hive
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place == None:
            for bee1 in my_place.bees[:]:
                bee1.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.real_queen != False:
            ...
            next_place, prev_place = self.place, self.place
            while next_place:
                if next_place.ant and next_place.ant.buffable:
                    if type(next_place.ant) == BodyguardAnt:
                        if next_place.ant.ant != self:
                            next_place.ant.ant.damage *= 2
                            next_place.ant.ant.buffable = False
                    next_place.ant.damage *= 2 
                    next_place.ant.buffable = False
                next_place = next_place.exit
            while prev_place:
                if prev_place.ant and prev_place.ant.buffable:
                    if type(prev_place.ant) == BodyguardAnt:
                        if prev_place.ant.ant != self:
                            prev_place.ant.ant.damage *= 2
                            prev_place.ant.ant.buffable = False
                    prev_place.ant.damage *= 2 
                    prev_place.ant.buffable = False
                prev_place = prev_place.entrance
        else:
